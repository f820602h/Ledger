  ...
  <Module class="col-12 col-md-auto mt-4">
    <div slot="body" class="calendar px-3 py-4">
      ...
      <div class="week d-flex flex-wrap mt-2 mx-auto">
        <p class="mb-0 p-2 text-center font-weight-bold" v-for="text in week" :key="text">
          {{ text }}
        </p>
      </div>
      <hr class="my-2" />
...
export default {
  ...
  computed: {
    ...
    nextMonth() {
      let total =
        this.HowManyDay(this.datePick.year, this.datePick.month) + this.prevMonth > 35 ? 42 : 35;
      return total - (this.HowManyDay(this.datePick.year, this.datePick.month) + this.prevMonth);
    },
    dateTimestamp() {
      return new Date(
        `${this.datePick.year}/${this.datePick.month + 1}/${this.datePick.date}`
      ).getTime();
    }
  },
  watch: {
