  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button type="button" class="close" @click="toggleAddPopup(false)"><span>&times;</span></button>
      </div>
      <div class="modal-body">
        <form>
          ...
          <div class="input-group">
            ...
            <input type="text" class="form-control" id="date" placeholder="YYYY/MM/DD" v-model.trim="$v.newData.date.$model">
          </div>
          ...
          <div class="input-group mt-4">
            ...
            <input type="text" class="form-control" id="time" placeholder="00:00 - 23:59" v-model.trim="$v.newData.time.$model">
          </div>
          ...
          <div class="input-group mt-4">
            ...
            <select class="custom-select" id="sheet" name="sheet" v-model="$v.newData.sheet.$model" @change="newData.type = ''">
              <option value="" disabled>請選擇</option>
              <option value="income">收入</option>
              <option value="pay">支出</option>
            </select>
          </div>
          <p class="error text-danger" v-if="!newData.sheet && $v.newData.$dirty">請選擇此筆帳目的收支</p>

          <div class="input-group mt-4">
            <div class="input-group-prepend"><span class="input-group-text">分類</span></div>
            <select class="custom-select" id="type" name="type" v-model="$v.newData.type.$model">
              <option value="" disabled>請選擇</option>
              <option v-for="(type, index) in filterType[newData.sheet]" :key="index" :value="type">{{type}}</option>
            </select>
          </div>
          <p class="error text-danger" v-if="!newData.type && $v.newData.$dirty">請選擇此筆帳目的分類</p>

          <div class="input-group mt-4">
            <div class="input-group-prepend"><span class="input-group-text">金額</span></div>
            <input type="text" class="form-control" id="cost" v-model.trim="$v.newData.cost.$model" @blur="$v.newData.cost.$touch">
          ...
          <p class="error text-danger" v-if="!$v.newData.cost.numeric">請避免輸入非數字之符號</p>
          ...
          <div class="input-group mt-4">
            ...
            <input type="text" class="form-control" id="description" v-model.trim="$v.newData.description.$model"/>
          </div>
          <p class="error text-danger" v-if="!newData.description && $v.newData.$dirty">請簡單說明此筆帳目</p>
          <p class="error text-danger" v-if="!$v.newData.description.maxLength">已超過50個字元</p>

        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" @click="toggleAddPopup(false)">取消</button>
