...
export default new Vuex.Store({
  ...
  actions: {
    SIGNUP({ commit, dispatch }, payload) {
      ...
      axios.post(`${process.env.VUE_APP_URL}ledger/signup`, payload).then(res => {
        if (res.data.success) {
    ...
    LOGIN({ commit, dispatch }, payload) {
      commit("SET_LOADING", true);
      axios.post(`${process.env.VUE_APP_URL}ledger/login`, payload).then(res => {
        if (res.data.success) {
    ...
    LOGOUT({ commit }, payload) {
      axios.get(`${process.env.VUE_APP_URL}ledger/logout`, payload).then(res => {
        if (res.data.success) {
          commit("SET_LOGIN_STATE", false);
          router.push("/");
        }
    ...
    INIT_DATA({ commit, dispatch }) {
      return new Promise(resolve => {
        ...
        axios.get(`${process.env.VUE_APP_URL}ledger/`).then(res => {
    ...
    ADD_NEW_DATA({ state, commit }, data) {
      ...
      newLedger.push(data);
      axios.post(`${process.env.VUE_APP_URL}ledger`, { newLedger, newSave }).then(() => {
    ...
    RESET_SAVE({ commit }, newSave) {
      axios.post(`${process.env.VUE_APP_URL}ledger/save`, { newSave: Number(newSave) }).then(() => {
        commit("SET_SAVE", newSave);
      });
    }
  },
